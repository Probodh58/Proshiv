...
do
        ...
        then
                ...
                then
                        ...
                        aws --profile "pt" ec2 create-tags --resources $instanceID --tags Key=ScheduleStop,Value=$incTime
                fi
                if [ "$action" == "sub" ]
                then
                        echo "instanceID : $instanceID - - - Previous Stop Time : $stopTime - - - Current Stop time : $DecTime"
                        aws --profile "pt" ec2 create-tags --resources $instanceID --tags Key=ScheduleStop,Value=$DecTime
                fi
