...
do
        ...
        then
                ...
                then
                        ...
                        aws --profile "qa" ec2 create-tags --resources $instanceID --tags Key=ScheduleStart,Value=$incTime
                fi
                if [ "$action" == "sub" ]
                then
                        echo "instanceID : $instanceID - - - Previous Start Time : $startTime - - - Current Start time : $DecTime"
                        aws --profile "qa" ec2 create-tags --resources $instanceID --tags Key=ScheduleStart,Value=$DecTime
                fi
